...
<?php 
 ...
 $q_pass = $conn->query("SELECT inspection.inspection_id, merchant.business_name,
                                                          inspection.date,inspection.status, admin.inspection_date, merchant.business_name
                                                 FROM merchant
                                                 RIGHT JOIN inspection
                                                 ON merchant.merchant_id = inspection.merchant_id 
                                                 RIGHT JOIN admin
                                                 ON admin.admin_id = inspection.admin_id 
                                                 WHERE inspection.date > DATE_SUB(admin.inspection_date, INTERVAL 6 MONTH) && inspection.status = 'Passed'
                                                 GROUP BY merchant.merchant_id") or die(mysqli_error());
    $f_pass = $q_pass->fetch_array(); 
    $count_pass = mysqli_num_rows($q_pass);
 ?>
                     ...
                     <div class="col-xl-3 col-sm-4 col-4 mb-4">
                        <div class="card">
                          <div class="card-body">
                            <div class="d-flex justify-content-between px-md-1">
                              <div class="align-self-center">
                                <i class="fas fa-spinner text-info fa-4x"></i>
                              </div>
                              <div class="text-end">
 ...
 <div class="col-lg-8" >
    <div class="card" style="height:429px;">
        <div class="card-body" >                                   
             <center><h4> <?php 
...
<a  class = "btn btn-info" href = "list.php"
 style="width:210px;color:white;margin-top:20px;margin-left:55px;">
 <i class = "fas fa-eye"></i>
         ...
         <br>           
             ...
             <center><h4>  SUMMARY LIST OF WATER REFILLING STATION 

                 </h4>
                 </center>  <br>
            <table id="TW" class="display" style="width:100%" >
              <thead>
                <tr>
                <th class="dada">ID</th>
                <th class="border-top-0">BARANGAY</th>
                 <th class="border-top-0">TOTAL</th>
                    ...
                    <tbody>
                          <?php  
                              $query = $conn->query("SELECT COUNT(barangay) as total  ,barangay, merchant_id FROM merchant
                WHERE status = 'approved'         
                GROUP BY barangay
                ORDER BY  length(barangay) ,barangay") or die(mysqli_error());
...
$c_done = mysqli_query($conn," SELECT  COUNT(DISTINCT merchant.merchant_id), merchant.merchant_id, merchant.business_name, merchant.status, inspection.date ,admin.inspection_date
                                 FROM inspection 
                                 RIGHT JOIN merchant on inspection.merchant_id= merchant.merchant_id
                                 RIGHT JOIN admin ON admin.admin_id = inspection.admin_id 
                                 WHERE merchant.status = 'approved' &&  inspection.date > DATE_SUB(admin.inspection_date, INTERVAL 6 MONTH)
...
<script type="text/javascript">
  ...
  "bInfo": false,
    ...
    tablee.on( 'order.dt search.dt', function () {
        ...
        tablee.cells(null, 0, {search:'applied', order:'applied'}).every( function (cell) {
            this.data(i++);
        } );
    } ).draw();
